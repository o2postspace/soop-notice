...
module.exports = async function handler(req, res) {
  ...
  // 오늘 (KST 기준) + offset — 하루 기준: 06:00 ~ 다음날 06:00
  const now = new Date();
  const kstNow = new Date(now.getTime() + 9 * 60 * 60 * 1000);
  // 현재 KST가 06시 이전이면 전날로 판단
  ...
  targetDay.setUTCHours(6, 0, 0, 0); // 당일 06:00 KST

  const dayEnd = new Date(targetDay);
  dayEnd.setUTCDate(dayEnd.getUTCDate() + 1); // 다음날 06:00 KST
